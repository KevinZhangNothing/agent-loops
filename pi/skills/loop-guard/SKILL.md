...
Only for **fix-capable** patterns (ci-sweeper, pr-babysitter,
dependency-sweeper, post-merge-cleanup). **Report-only** patterns
(daily-triage, issue-triage, changelog-drafter) skip it — they don't mutate
code, so there is nothing to break against. See `patterns/registry.yaml` for
the canonical `skills` list per pattern.

## The ledger

`loop-ledger.json` records the loop's goal, its pattern/level (seeded by
`loop-init`), and one entry per attempt:
...
- `loop-constraints` — honors "escalate after N attempts" (default 3, set in
  `loop-constraints.md`); this skill makes it mechanical.
