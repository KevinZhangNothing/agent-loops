...
agent-loops 项目现已**完全兼容 pi coding agent**。所有必要的技能、工作流、MCP 配置和文档已创建、修复并验证通过。
...
| `pi/mcp.json` | 94 | MCP 服务器配置模板（1 个真实 MCP 服务器 / 10 shortcuts） |
...
| 7 | mcp.json 注册了 3 个伪 MCP 服务器（`LoopAudit`/`LoopInit`/`LoopCost` 实际是 CLI） | 移除伪 MCP 服务器，改用 shortcuts；`loop_estimate_cost` 已在 LoopEngineering 服务器内 |
...
| `+loop-score-json` | `npx @kevinzhangnothing/loop-audit . --json` |
| `+loop-badge` | `npx @kevinzhangnothing/loop-audit . --badge` |
| `+loop-init` | `npx @kevinzhangnothing/loop-init . -p daily-triage -t claude` |
| `+loop-init-dry-run` | `npx @kevinzhangnothing/loop-init . -p daily-triage -t claude --dry-run` |
| `+loop-cost` | `npx @kevinzhangnothing/loop-cost --pattern daily-triage --level L1` |
| `+loop-cost-json` | `npx @kevinzhangnothing/loop-cost --pattern daily-triage --level L1 --json` |
...
| 服务器 | Resources | Tools | 类型 |
|--------|-----------|-------|------|
| `LoopEngineering` | 8 个（`loop://*`） | 8 个（含 `loop_estimate_cost`） | 真实 MCP 服务器 |
| `+loop-score` 等 10 shortcuts | — | — | CLI 快捷方式（替代伪 MCP 服务器） |
...
```
Loop Readiness Audit — agent-loops
═════════════════════════════════════════════════
...
✅ **agent-loops 现已完全兼容 pi coding agent (v2)**
