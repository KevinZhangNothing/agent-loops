...
将 agent-loops 集成到 **pi coding agent** 的完整指南。

> **pi** 是一个强大的 coding agent 框架，支持 Skills、MCP 服务器、Workflows 和 Shortcuts。本指南帮助你充分利用 agent-loops 与 pi 的集成。
...
```bash
# 从 agent-loops 复制 pi 配置
cp /path/to/agent-loops/pi/mcp.json ~/.pi/agent/

# 或者在你的项目中使用
cp /path/to/agent-loops/pi/ .
```

### 2. 配置 MCP 服务器

编辑 `~/.pi/agent/mcp.json`，添加 Agent Loops MCP 服务器：
...
推荐使用 `install.sh` 一键安装所有技能：

```bash
bash /path/to/agent-loops/pi/install.sh
```

或手动复制（注意全部 12 个 skill，不限于 `loop-*` 前缀）：

```bash
# 复制所有 skills 到 pi 技能目录
cp -r /path/to/agent-loops/pi/skills/. ~/.pi/agent/skills/

# 验证安装
ls ~/.pi/agent/skills/
# 应该看到：loop-audit, loop-init, loop-triage, loop-cost, loop-budget,
#          loop-verifier, minimal-fix, loop-sync, loop-guard,
#          ci-triage, pr-review-triage, rebase-and-clean
```
...
name: daily-triage
trigger:
  cron: '0 1 * * 1-5'  # 09:00 Asia/Shanghai (UTC+8)
level: L1
budget:
  daily_cap: 100000
```
...
| Tool | 描述 | 参数 |
|------|------|------|
| `loop_list_patterns` | 列出所有 patterns | 无 |
| `loop_list_skills` | 列出所有 skills | 无 |
| `loop_list_state_files` | 列出状态文件 | 无 |
| `loop_get_pattern` | 获取 pattern 详情 | `patternId` (必填) |
| `loop_get_skill` | 获取 skill 定义 | `skillName` (必填) |
| `loop_get_state` | 读取状态文件 | `stateFile` (可选，默认 `STATE.md`) |
| `loop_recommend_pattern` | 根据用例推荐 pattern | `useCase` (必填) |
| `loop_estimate_cost` | 估算 token 花费 | `patternId` (必填), `level` (必填, L1/L2/L3), `cadence` (可选) |

这些工具通过 MCP JSON-RPC 协议调用（不是 CLI 标志），例如：

```json
// 调用 loop_get_pattern
{ "patternId": "ci-sweeper" }

// 调用 loop_estimate_cost
{ "patternId": "daily-triage", "level": "L2" }
```
...
```bash
# 1. 查询可用 patterns（MCP 工具，无参数）
loop_list_patterns

# 2. 获取 pattern 详情（MCP 工具，参数 patternId: ci-sweeper）
loop_get_pattern
...
```bash
# 检查技能目录（列出所有 12 个 skill）
ls ~/.pi/agent/skills/

# 使用 install.sh 重新安装
bash /path/to/agent-loops/pi/install.sh

# 或手动复制所有 skill（注意: 不限于 loop-* 前缀）
cp -r /path/to/agent-loops/pi/skills/. ~/.pi/agent/skills/
```
